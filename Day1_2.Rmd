...
>
> 새로 생성된 변수가 반영된 데이터를 사용하기 위해선 `<-`를 이용해 새로운 이름으로 데이저를 명명하자.


```{r}
ex1 <- ex %>% mutate(BMI.pre = pre.weight/(Height^2)*10000) # 키의 단위는 m이므로 곱하기 10000 
ex1 %>% filter(BMI.pre<=18.5)%>% mutate(BMI.cate="저체중")
```
...
head(mpg)
```
![](C:/Users/stats/Documents/R 중급강좌 (통계교육원)/R_beginner-master/fig1.png)
![](C:/Users/stats/Documents/R 중급강좌 (통계교육원)/R_beginner-master/fig2.png)
![](C:/Users/stats/Documents/R 중급강좌 (통계교육원)/R_beginner-master/fig3.png)
